...
def read_params(filename, n_variables):
    """Read Dakota’s annotated params file and collect design variable values.

    Supports:
      - continuous_design:            cdv_1, cdv_2, ...
      - discrete_design_range integer: ddriv_1, ddriv_2, ...
      - discrete_design_range real:    ddrv_1, ddrv_2, ...
      - discrete_design_set integer:   ddsiv_1, ...
      - discrete_design_set real:      ddsrv_1, ...
    """
    xs = []
    pat = re.compile(
        r'^\s*'                                  # leading spaces
        ...
        r'\s+'                                   # spaces
        r'(?:cdv|ddriv|ddrv|ddsiv|ddsrv)_'       # supported var tags
        r'\d+\s*$',                               # index and optional trailing spaces
        flags=re.IGNORECASE
    )
